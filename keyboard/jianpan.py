...
pygame.display.set_caption("keyboard practice")
...
def display_screen(font, color, current, correct_text, correct_answer):
...
def display_screen_gameover(score, count):
    screen.fill(background)
...
def main():
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_RETURN:
                    ...
                    count +=1
                    count = 0 if  game_over else count
        ...
        if game_over:
            display_screen_gameover(score, count)

        else:
            display_screen(font2, color, current, correct_text, correct_answer)
